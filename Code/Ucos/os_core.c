...
static void OS_InitTimeChain(void);
...
#ifdef OS_TIME_CHAIN
    OSTimeDlyLink(OSTCBCur, timeout);
#else
...
//        pTimechainNow = OSTimeChainList;
        while (pTimechainNow != (OS_TIMECHAIN *) 0 && pTimechainNow->dly == 0u) {
            ptcb = pTimechainNow->tcb;
            ptcb->OSTCBDly = 0;
            if ((ptcb->OSTCBStat & OS_STAT_PEND_ANY) != OS_STAT_RDY) {
                ptcb->OSTCBStat &= (INT8U)
                        ~(INT8U) OS_STAT_PEND_ANY;   /* Yes, Clear status flag   */
            ...
            if (OSTimeChainList != 0) {
                OSTimeChainList->OSTimeChainPrev = 0;
            }
...
static void OS_InitTimeChain(void) {
    INT8U ix;
    INT8U ix_next;
    OS_TIMECHAIN *ptimechain1;
    OS_TIMECHAIN *ptimechain2;


    OS_MemClr((INT8U *) &OSTimeChainTbl[0], sizeof(OSTimeChainTbl));
    for (ix = 0u; ix < (OS_MAX_TASKS + OS_N_SYS_TASKS - 1u); ix++) {
        ix_next = ix + 1u;
        ptimechain1 = &OSTimeChainTbl[ix];
        ptimechain2 = &OSTimeChainTbl[ix_next];
        ptimechain1->OSTimeChainNext = ptimechain2;
    }
    ptimechain1 = &OSTimeChainTbl[ix];
    ptimechain1->OSTimeChainNext = (OS_TIMECHAIN *) 0;
    OSTimeChainList = (OS_TIMECHAIN *) 0;
    OSTimeChainFreeList = &OSTimeChainTbl[0];
